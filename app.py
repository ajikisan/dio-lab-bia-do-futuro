...
with gr.Blocks(theme=custom_theme) as demo:
    ...
    chatbot = gr.Chatbot(label="Capivara Financeira no Reino das Moedas", type="messages", allow_tags=False)
    ...
    entrada.submit(fluxo, inputs=[entrada, chatbot], outputs=[entrada, chatbot, grafico_out, audio_out])
    enviar_btn.click(fluxo, inputs=[entrada, chatbot], outputs=[entrada, chatbot, grafico_out, audio_out])
    limpar_btn.click(limpar_ui, inputs=[], outputs=[entrada, chatbot, grafico_out, audio_out])

    # 📊 Botões simples (só gráfico)
    ...
    # 📊 Botões completos (chat + áudio + gráfico)
    grafico_trans_full_btn.click(fn=lambda: responder("gráfico transações", [], False), inputs=[], outputs=[entrada, chatbot, grafico_out, audio_out])
    grafico_atend_full_btn.click(fn=lambda: responder("atendimento", [], False), inputs=[], outputs=[entrada, chatbot, grafico_out, audio_out])
